...
void Inventory::searchProduct(const std::string& keyword) const {
    ...
    for (const auto& p : products) {
        ...
        if (nameLower.find(keywordLower) != std::string::npos || std::to_string(p.id) == keyword) {
            std::cout << p.id << " | " << p.name << " | " << p.quantity << " | $" << p.price << "\n";
            found = true;
        }
    }
...
void Inventory::sortInventoryByPrice() const {
...
void Inventory::importFromCSV(const std::string& csvFile) {
    ...
    if (!in.is_open()) {
        std::cout << "Failed to open " << csvFile << "\n";
        return;
    }
    ...
    while (getline(in, line)) {
        std::stringstream ss(line);
        Product p;
        std::string idStr, qtyStr, priceStr;
        getline(ss, idStr, ','); p.id = std::stoi(idStr);
        getline(ss, p.name, ',');
        getline(ss, qtyStr, ','); p.quantity = std::stoi(qtyStr);
        getline(ss, priceStr); p.price = std::stod(priceStr);
    ...
    save();
    std::cout << "Imported " << imported << " new products from CSV.\n";
}
